...
class TorchCMult(TorchDistribution):
    ...
    def __init__(self, total_count=1, probs=None, logits=None, validate_args=None):
        ...
        batch_shape = self._categorical.batch_shape + self._categorical.param_shape[-1:]
        super(CMult, self).__init__(batch_shape, #event_shape,
                                    validate_args=validate_args)

    def expand(self, batch_shape, _instance=None):
        new = self._get_checked_instance(CMult, _instance)
        batch_shape = torch.Size(batch_shape)
        new.total_count = self.total_count
        new._categorical = self._categorical.expand(batch_shape[:-1])
        super(CMult, new).__init__(batch_shape, #self.event_shape,
                                validate_args=False)
    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        samples = self._categorical.sample(torch.Size((int(self.total_count),)) + sample_shape)
        ...
        print("cmult sample:",counts.shape)
        return counts.type_as(self.probs)

    def log_prob(self, value):
        print("cmult lp:",value)
