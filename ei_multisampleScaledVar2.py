        ...
        with all_sampled_ps() as p_tensor:
            logits = (
                pyro.sample(f'{iter}logits',
                    dist.Normal(logit_centers, sdprc/sq_E_y_cond_on_gamma).to_event(2))
                ) #eprc.size() == [P,R,C] because plate dimension happens on left

        dp("sq_E_y_cond_on_gamma",torch.mean(sq_E_y_cond_on_gamma),
                torch.std((logits-logit_centers)*sq_E_y_cond_on_gamma),
                torch.std((logits-logit_centers)),
                sizes(sq_E_y_cond_on_gamma,logits-logit_centers))
...
def guide(data, scale, do_print=False, inits=dict(), nsamps = 1, icky_sigma=ICKY_SIGMA,
    ...
    #for p in prepare_ps:#pyro.plate('precinctsg2', P):
        ...
        lr_var_of_like = (sq_E_y_cond_on_gamma**2) * (nsUbyRby1-ystars)/(ystars*nsUbyRby1)
        ...
        eprcstars = (
                ...
                (1/lr_var_of_like/SDS_TO_SHRINK_BY + 1/sdprc**2)    /           sq_E_y_cond_on_gamma)

        dp("sds:",torch.mean(sq_E_y_cond_on_gamma),
                (logresidual_raw/sq_E_y_cond_on_gamma).std(),
                logresidual_raw.std(),sdprc,eprcstars.std())
        if do_print:
            print("sds:",torch.mean(sq_E_y_cond_on_gamma),logresidual_raw.std(),sdprc,eprcstars.std())
