...
* Hessian thereof. Up to boosting, unbiased for both conditional precision $G$ and marginal precision $([\mathcal{I}_p(\bm{\theta}^*)^{-1}]_{\Gamma,\Gamma})^{-1}=G-\sum_i C_iU_i^{-1}C_i^T$
...
\begin{tikzpicture}
  ...
  \node[latent, diamond, below=2 of hats, xshift=.5cm]            (sighat) {$\orange{\sigma_{\nu}^*}$};
  
  \node[latent, diamond, aspect=2.5, below=3 of hats]            (Iparams) {$\mathscr{I}^{-1}(\bm{\gamma}^*)$};
  \factor[left=of Iparams] {paramdist} {left:$\mathcal{N}$} {} {};
  \node[latent, below=of paramdist]            (params) {$\orange{\bm{\gamma}}$};
  

  
  \node[latent, diamond, aspect=3,  below=3 of hats,  align=center, xshift=3.5cm]  (Ifull) {$\mathscr{I}^{-1}(\bm{\gamma}^*,\bm{\lambda}^*_{u})$};
...
<!--   \node[latent, diamond, aspect=2.5, below=3.4 of hats]            (Iparams) {$\mathscr{I}^{-1}(\bm{\gamma}^*)$}; -->
<!--   \factor[left=of Iparams, xshift = -1cm] {paramdist} {left:$\mathcal{N}$} {} {}; -->
<!--   \node[latent, below=.8 of Iparams]            (params) {$\bm{\gamma}$}; -->



<!--   \node[latent, diamond, aspect=2.5, right=.33 of Iparams]  (Ifull) {$\mathscr{I}^{-1}(\bm{\gamma}^*,\bm{W}^*_{u})$}; -->
