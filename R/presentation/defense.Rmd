...
\begin{tikzpicture}
  ...
  \node[const, below=2.2 of level, align=center]          (global) {\orange{Global voting propensities}\\\orange{(for each race)}};
  \node[const, below=4.2 of level, align=center]          (latent) {\green{Precinct-level variation}\\\green{(for each race)}};
  \node[const, below=8.2 of level, align=center]          (unobserved) {\blue{Precinct vote totals}\\\blue{by race \& candidate}\\\blue{(unobserved)}};
  \node[const, below=10.2 of level, align=center]          (observed) {\purple{Precinct vote totals}\\\purple{by candidate (observed)}};
  \node[const, below=11.2 of level]          (issues) {Advantages};
  \node[const, below=12.5 of level, align=center]          (issues) {Issues\\ \\ \\ \\};
  ...
  % King
  \node[const, right=2.5 of ER]           (King) {King's EI};
  ...
  Extensible!};
  %\node[const, align=center, below=12.5 of us]      (uscon) {Few — let me\\describe it first};
