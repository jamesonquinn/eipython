...
note("You will notice a few changes to what I sent you, particularly in the chapter 3 results; I will point them out as we go along\\\\Talk about Mira\\\\the major motivating ideas (idea of Laplace family, idea of applying VI to EI) come from me\\\\I did all the coding in Pyro\\\\otherwise Mira is equal collaborator and coauthor\\\\we checked that this is allowed
")
```
...
The Laplace guide family $\mathcal{L}_{\Theta\times\Psi} (p,f_\Psi)$ is the set of $d$-dimesnional normal distributions $\{q_{\bm{\theta}^*,\bm{\psi}}:\bm{\theta}^*\in\Theta,\;\bm{\psi}\in\Psi\}$, where $q_{\bm{\theta}^*,\bm{\psi}}$ has mean $\bm{\theta}^*$ and precision matrix $f_{\bm{\psi}} \left(\I_p(\bm{\theta}^*)\right)$.} 
