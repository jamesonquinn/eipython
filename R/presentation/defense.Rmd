...
If integral is over hi-D space, numerical methods for estimating will have unacceptably high variance. 
So you need tricks.")
```
...
```{r, out.width = "60px", echo=FALSE, results='asis'}
note("Ch. 2: The VI framework is to assume the posterior is well-approximated... construct a new guide family that's able to...
...
```{r, out.width = "60px", echo=FALSE, results='asis'}
note("minimizing KL-divergence, or without normalization, the ELBO)
Note model parameters here and guide parameters there
Optimizing, but over distributions, so entropy; 
hill-climing means automatic differentiation, so...")
```
...
Let $p(\bm{\theta})$ be a probability density over $\mathbb{R}^d$. 

Let $\Theta \subseteq \mathbb{R}^d$, $\Psi \subseteq \mathbb{R}^d_+$, and let $f_\Psi$ be a boosting family. 

Laplace guide: $q_{\bm{\theta}^*,\bm{\psi}}(\bm{\theta})$, a $d$-dimensional Gaussian with mean $\bm{\theta}^*$ and precision precision matrix $f_{\bm{\psi}} \left(\mathcal{I}_p(\bm{\theta}^*)\right)$.
...
Laplace guide parameters: $\bm{\gamma}^*,\bm{\lambda}^*,\bm{\psi}_\gamma,\bm{\psi}_\lambda$
...
## Latent variable models: Hessians
...
with
$$p_\mathcal{S}(\bm{\theta}_\mathcal{S},\bm{x}_\mathcal{S}) := \mathrm{log}p\bm{\gamma})+\frac{1}{\pi_{i}}\sum_{i\in \mathcal{S}} \Big[\mathrm{log}p\bm{\lambda}_{i}|\bm{\gamma})\; +\mathrm{log}p\bm{x}_{i}|\bm{\lambda}_{i},\bm{\gamma})\Big]$$
With Laplace guide, this makes:

* Posterior log density: unbiased (as in meanfield)
* Hessian: Up to boosting, unbiased for both conditional precision and "marginal precision" (inverse of marginal covariance).
* ELBO and ELBO gradient: Not unbiased (unlike meanfield)

```{r, out.width = "60px", echo=FALSE, results='asis'}
note("If cheap way to predict bigger terms, weights; we haven't implemented
...
Lower-dimensional subfamily of a Laplace family: reduce the number of guide parameters by setting $\bm{\lambda^*}$ to $f(\bm{\gamma^*})$
...
\draw[red,fill=red] (11.75,2.25) circle (.5ex);
\node[] at (11.75,1.85)   (c) {$\dot{\bm{Y}}$};

\node[] at (12, -1.1)   (c) {$\mathcal{Y}_u$};

%polytope
\draw[style=-] (13.5,0) -- +(0,1) node[right = .3cm,anchor=base,midway,rotate=90] {$\textcolor{black}{y_{p11}<100}$};
\draw[style=-] (10,3.5) -- +(0,1) node[left = .2cm,anchor=base,midway,rotate=90] {$\textcolor{black}{y_{p11}}>0$};
