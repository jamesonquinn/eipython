...
#temp: no 400
SUBSAMPLE_NS = c(400,100,50,10,44) 
subsample_line_types = c(2,1,3,4,1)#1:length(SUBSAMPLE_NS)
names(subsample_line_types) = as.character(SUBSAMPLE_NS)
subsample_labels = as.character(SUBSAMPLE_NS)
names(subsample_labels) = as.character(SUBSAMPLE_NS)
...
EXTRA_FORMATTING = guides(linetype = FALSE, size=FALSE) #geom_blank()
...
fat = TRUE
if (fat) {
  fat_metrics = get_metrics_for(ndom_fat_params,dographs=all_guides[c(1,4)])
