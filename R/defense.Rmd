...
problem with high-dimensional posteriors
VERY brief mention of ch 1
Ch2 : new method
Ch 3: application of method, extension of existing models
You will notice a few changes to what I sent you, I will point them out as we go along
Talk about Mira
the major motivating ideas (idea of Laplace family, idea of applying VI to EI) come from me
I did all the coding in Pyro (which is most of the coding and was quite non-trivial)
otherwise Mira is equal collaborator and coauthor
we checked that this is allowed

## Variational Inference

Slide has the basic equations
...
This is cutting edge stuff in ML (esp black box VI)
Pyro (is being developed as we speak)

## Pyro

Explain automatic differentiation
All the impressive software engineering I had to do
Luke: should this slide be non-hidden, as a way to sell myself more?

## Problem with existing (mainstream) VI method

mention non-black-box VI
mean field (for example normal)
can’t capture posterior correlations, systematically underestimates posterior marginals
with picture!

## Introduce Laplace family VI (1)

Normal guide families:
full normal is too big
mean field is a subfamily that doesn’t actually contain any good approximations
we want subfamily that contains at least some good approximations without being too big
Aha! Laplace approximation!
Intuitive explanation of Laplace family
...
All formulas in all their gory detail
...
Define, including defining Gamma and Theta
This is why we need a high-dimensional solution.

## Block arrowhead matrices

Show block-arrowhead matrix, say it’s easy to sample from and easy to boost
(don’t write anything down for this, just point)

## Additional methods for LV models: subsampling

Explain subsampling
Mention weights, say we haven’t yet implemented them anywhere
...
restricting to subspace of the guide where latents are approximate MLEs relative to globals, 
guide parameters are now just gamma, psi_gamma, psi_lambda
(possibly mention Newton’s method, but Mira thinks not)
(Say that they go together nicely, but don’t go into detail)
