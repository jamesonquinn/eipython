...
data_dir = "../ei_post_results_MSEbiasterm/"
...
asPercentStr = function(val,ndigits=3) {
  return(paste0(print(formatC(signif(val,digits=ndigits), digits=ndigits,format="fg", flag="#")),"\\%"))
}

makeResultTable = function(samp.filename,fit.filename) {
    samp=fread(samp.filename)
    msamp = as.matrix(samp)
    VIsamp= msamp[,c(1,4,7,2,5,8,3,6,9)+7]
    sparts = strsplit(samp.filename,"_")[[1]]
    scenario.filename = paste(paste0(data_dir,"/scenario"),sparts[8],0,sparts[10],sep="_")
    ...
    for (r in 1:3) {
      ...
      for (c in 1:3) {
        ...
        results[baserow+2,basecol+1] = asPercentStr(2*sd(rjkts),2)
