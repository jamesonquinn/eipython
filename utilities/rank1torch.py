...
def optimize_QU(U,R,C,
        pi, #U x R x C
        v, #U x C
        d, #U x R
        tolerance=tolerance,iters=3):

    M,D = make_MD(R,C) #TODO: should precalculate; fix later.

    # Other stuff we'll need
    v_d = torch.cat((v,d),1).reshape(-1,R+C,1) #TODO: this stuff can also be precalculated
    ind = torch.matmul(d.reshape(R,1),v.reshape(-1,1,C)) #TODO: this stuff can also be precalculated

    # OK, let's get started!
    # Start with a bad guess for beta and a very high error...
    beta = torch.ones(U,1,C)
    ...
    while (i<maxiters and (torch.any(errorQ > tolerance) or torch.any(errorQ < -tolerance))):
        ...
        #adjust alpha based on current beta
        pi_beta =  torch.matmul(pi,torch.diag(beta.view(-1)))
        M_alpha = torch.cat((pi_beta.permute(1,0),torch.diag(rowsum(pi_beta))),0)
        alpha, lu_ = torch.solve(torch.matmul(M_alpha.permute(1,0),v_d), torch.matmul(M_alpha.permute(1,0),M_alpha))

        #adjust beta based on current alpha
        alpha_pi = torch.matmul(torch.diag(alpha.view(-1)),pi)
        M_beta = torch.cat((torch.diag(colsum(alpha_pi)),alpha_pi),0)
        beta, lu_ = torch.solve( torch.matmul(M_beta.permute(1,0),v_d), torch.matmul(M_beta.permute(1,0),M_beta))

        # figure out error
        Q = torch.matmul(torch.diag(alpha.view(-1)), torch.matmul(pi,torch.diag(beta.view(-1))))
        errorQ = torch.mv(D,(Q-ind).view(-1)).reshape(R,C)
        #print(f"Error in Q:\n{errorQ}")
