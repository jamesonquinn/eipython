...
class ArrowheadPrecision:
    ...
    def calc_lls(self):
        if self.lls is not None:
            return
        dp("calc_lls",[a.size() for a in [self.gg,self.gls[0],self.raw_lls[0],self.psig,self.psil]])
    ...
    def marginal_gg_cov(self):
        ...
        dp("inner si",self.gg.size(),inner.size(),self.vecgls.size(),transp.size())
