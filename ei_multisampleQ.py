...
def model(data=None, scale=1., include_nuisance=True, do_print=False, nsamps = 1, *args, **kwargs):
    """
    ...
    for isamp in range(nsamps):
        ...
        if include_nuisance:
            with all_sampled_ps() as p_tensor:
                logits = (
                    pyro.sample(f'{iter}logits', dist.Normal((ec + erc).expand(P,R,C), sdprc).to_event(2))
...
def guide(data, scale, include_nuisance=True, do_print=False, inits=dict(), nsamps = 1, icky_sigma=ICKY_SIGMA,
    ...
    #for p in prepare_ps:#pyro.plate('precinctsg2', P):
        ...
        if include_nuisance:
            ...
            lr_var_of_like = (nsUbyRby1-ystars)/(ystars*nsUbyRby1)
...
def weighted_incremental_variance(dataWeightPairs):
    ...
    # big_grad = big_grad,
    # adjusted_means = adjusted_means
def sampleYs(fit,data,n,previousSamps = None,weightToUndo=1.,indices=None, icky_sigma=ICKY_SIGMA, use_grad=False):
    ba = fit["big_arrow"]
    am = fit["all_means"]
    G,L = ba.G, ba.L
    R, C = (data.R,data.C)
    ...
    for u in indices:
        ...
        if ignore_nu:
            ...
            lmean = lstar.unsqueeze(0) + torch.matmul(g_deltas.unsqueeze(1), torch.matmul(ba.gls[u],ll[u]))
...
def trainGuide(subsample_n = SUBSET_SIZE,
    ...
    data = createOrLoadScenario(dummydata,filebase=filebase,dversion=0)#dversion)
    N = data.U
    scale = N / subsample_n
    #dp(data)
